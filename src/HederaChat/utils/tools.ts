...
function convertTransactionId ( transactionId: string )
{
  // Split the string into two parts at the '@' symbol
  const [ accountPart, rest ] = transactionId.split( '@' );

  // Split the second part into two at the first '.' only
  const [ seconds, nanos ] = rest.split( '.' );

  // Join them together with '-' in the desired format
  return `${accountPart}-${seconds}-${nanos}`;
}